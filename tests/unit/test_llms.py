...
def test_to_lc_messages_converts_user():
...
def test_to_lc_messages_converts_system():
...
def test_to_lc_messages_converts_assistant():
    from langchain_core.messages import AIMessage
...
def test_extract_text_from_aimessage():
...
def test_call_with_prompt(mock_chat_llm):
...
def test_call_with_messages(mock_chat_llm):
    lm = LangChainLM(mock_chat_llm)
...
def test_call_raises_without_prompt_or_messages(mock_chat_llm):
    lm = LangChainLM(mock_chat_llm)
    with pytest.raises(ValueError, match="Either"):
        lm()


def test_history_is_recorded(mock_chat_llm):
    lm = LangChainLM(mock_chat_llm)
    lm(prompt="Q1")
    lm(prompt="Q2")
...
def test_inspect_history_returns_last_n(mock_chat_llm):
    lm = LangChainLM(mock_chat_llm)
    for i in range(5):
        lm(prompt=f"Q{i}")
...
def test_usage_metadata(mock_chat_llm):
    lm = LangChainLM(mock_chat_llm)
    lm(prompt="test")
...
def test_call_completion_model(mock_completion_llm):
...
def test_temperature_and_max_tokens_forwarded(mock_chat_llm):
