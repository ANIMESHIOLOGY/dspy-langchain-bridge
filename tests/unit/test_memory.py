...
def test_get_history_returns_list(mock_lc_memory):
    adapter = LangChainMemoryAdapter(mock_lc_memory)
    history = adapter.get_history()
    assert isinstance(history, list)


def test_get_history_correct_roles(mock_lc_memory):
    adapter = LangChainMemoryAdapter(mock_lc_memory)
    history = adapter.get_history()
...
def test_get_history_correct_content(mock_lc_memory):
    adapter = LangChainMemoryAdapter(mock_lc_memory)
    history = adapter.get_history()
...
def test_get_history_custom_roles(mock_lc_memory):
...
def test_get_history_system_message():
    memory = MagicMock()
...
def test_save_turn_calls_save_context(mock_lc_memory):
    adapter = LangChainMemoryAdapter(mock_lc_memory)
...
def test_save_turn_no_method_does_not_raise():
...
def test_clear_calls_memory_clear(mock_lc_memory):
    adapter = LangChainMemoryAdapter(mock_lc_memory)
    adapter.clear()
...
def test_get_history_from_load_memory_variables():
