...
def test_langchain_retriever_returns_prediction(mock_lc_retriever) -> None:
...
def test_langchain_retriever_respects_k(mock_lc_retriever) -> None:
...
def test_langchain_retriever_multiple_queries(mock_lc_retriever) -> None:
    retriever = LangChainRetriever(mock_lc_retriever, k=2)
...
def test_langchain_retriever_passages_are_strings(mock_lc_retriever) -> None:
    retriever = LangChainRetriever(mock_lc_retriever, k=3)
...
def test_langchain_retriever_repr(mock_lc_retriever) -> None:
    retriever = LangChainRetriever(mock_lc_retriever)
    assert "LangChainRetriever" in repr(retriever)


@pytest.mark.asyncio
async def test_langchain_retriever_acall(mock_lc_retriever) -> None:
...
def test_dspy_retriever_returns_documents(mock_dspy_retrieve) -> None:
    retriever = DSPyRetriever(mock_dspy_retrieve)
    docs = retriever.invoke("What is the capital of France?")
    assert len(docs) > 0


def test_dspy_retriever_returns_document_objects(mock_dspy_retrieve) -> None:
...
def test_dspy_retriever_page_content_populated(mock_dspy_retrieve) -> None:
    retriever = DSPyRetriever(mock_dspy_retrieve)
...
def test_dspy_retriever_legacy_interface(mock_dspy_retrieve) -> None:
...
def test_dspy_retriever_repr(mock_dspy_retrieve) -> None:
    retriever = DSPyRetriever(mock_dspy_retrieve)
    assert "DSPyRetriever" in repr(retriever)


@pytest.mark.asyncio
async def test_dspy_retriever_ainvoke(mock_dspy_retrieve) -> None:
    retriever = DSPyRetriever(mock_dspy_retrieve)
